...
HTTPS_GIT := github.com/adjacent-labs/infra.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=infra \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=infrad \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/infrad ./cmd/infrad
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/infrad.exe ./cmd/infrad
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/infrad
...
draw-deps:
	...
	@goviz -i ./cmd/infrad -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c infra --local -f chains.yaml
endif
...
set-testnet-configs:
	infrad config set client chain-id chainid-1
	infrad config set client keyring-backend test
	infrad config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | infrad keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | infrad keys add acc1 --recover`
