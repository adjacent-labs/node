...
# then log output of file /code/bin/infrad
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/infrad \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/infrad | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/infrad /usr/bin/infrad
